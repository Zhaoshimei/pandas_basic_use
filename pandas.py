# -*- coding: utf-8 -*-
"""
Created on Tue Jan 17 23:51:37 2017
...
data = {'state':['ok', 'ok', 'good', 'bad'],
        ...
        'pop':[3.7, 3.6, 2.4, 0.9]}
print DataFrame(data) # 行索引index默认 
'''
   pop state  year
0  3.7    ok  2000
1  3.6    ok  2001
...
3  0.9   bad  2003
'''
print DataFrame(data, columns = ['year', 'state', 'pop','debt']) # 指定列顺序,不匹配的列为NaN
'''
   year state  pop
0  2000    ok  3.7
1  2001    ok  3.6
...
dtype: float64
'''
print x.reindex(['a', 'b', 'c', 'd'], fill_value = 0)  # 指定不存在元素NaN的默认值
'''
a    4
b    7
c    5
...
d  14  15
'''
print x.drop('a', axis = 0) # 在行的维度上删除
'''
    A   B   C   D
b   4   5   6   7
c   8   9  10  11
...
c  1    4
   2    5
'''
print data[:2] # 数组索引不区分标签
'''
a  1    0
   2    1
'''
print data.unstack() #将Series转换为DataFrame
'''
   1  2
a  0  1
b  2  3
...
d  6  7
'''
print data.unstack().stack() # 将DataFrame转换回Series
'''
a  1    0
   2    1
b  1    2
...
print '直接用MultiIndex创建层次索引结构index'
...
6  6  1  two  3
'''
print frame.set_index(['c', 'd'])  # 把c/d列索引变成行索引
'''
       a  b
c   d      
one 0  0  7
